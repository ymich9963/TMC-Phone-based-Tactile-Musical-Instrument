...
public class EnvelopeADSR extends SynthUnit {
    ...
    private static float mSustainLevel; //had to change this to static in order for the static function to work
    private float mDecayRate;
    private float mCurrent;
    private int mSstate = IDLE;

    public EnvelopeADSR() { //try to change this with the CCs
        setAttackTime(0.003f); //used to be 0.003f
        setDecayTime(0.08f);
        setSustainLevel(0.3f); //used to be 0.3f
        setReleaseTime(1.0f);
    }
    ...
    //-------------------ADDITIONS-----------------------------------------
    //changed the function to static in order to access it from the SynthEngine class
    public static void setSustainLevel(float level) {
        if (level < 0.0f)
            level = 0.0f;
        mSustainLevel = level;
    }
